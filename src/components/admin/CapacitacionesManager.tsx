  ...
  return (
    <div>
      ...
      {tab === 'SESIONES' && (
        <>
          <div className="mb-4">
            <button onClick={() => setIsModalOpen(true)} className="w-full md:w-auto bg-brand-teal text-white px-4 py-2 rounded shadow hover:bg-brand-dark transition">
              + Nueva Fecha
            </button>
          </div>
          <div className="bg-white shadow rounded-lg overflow-hidden overflow-x-auto">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-brand-dark/50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6 overflow-y-auto max-h-[90vh]">
            <h3 className="text-xl font-bold font-heading mb-4 text-brand-dark">Programar Nueva Sesión</h3>
            ...
            <form onSubmit={handleCreateSession} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="date" name="endDate" className="w-full border border-brand-grey/30 rounded px-3 py-2 text-sm" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select name="modality" required className="w-full border border-brand-grey/30 rounded px-3 py-2 text-sm">
                ...
                <div>
                  ...
                  <input type="number" name="seatsTotal" required min="1" max="100" defaultValue="15" className="w-full border border-brand-grey/30 rounded px-3 py-2 text-sm" />
                </div>
              </div>

              <div>
                <label className="block text-sm font-bold text-brand-dark mb-1">Ubicación (Opcional)</label>
                <input type="text" name="location" placeholder="Ej: Centro de Capacitación Rancagua" className="w-full border border-brand-grey/30 rounded px-3 py-2 text-sm" />
              </div>

              <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-brand-light">
                <button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 text-sm text-brand-grey hover:bg-brand-light rounded transition">Cancelar</button>
